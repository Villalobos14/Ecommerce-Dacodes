export default function ErrorPage() {
    return (
        <>
            <main className="flex justify-center h-screen items-center bg-white px-6 py-24 sm:py-32 lg:px-8 ">
                <div className="text-center">
                    <p className="text-base font-semibold text-[#322017]">404</p>
                    <h1 className="mt-4 text-3xl font-bold tracking-tight text-gray-900 sm:text-5xl">Página no encontrada</h1>
                    <p className="mt-6 text-base leading-7 text-gray-600">Disculpa, no pudimos encontrar la página que buscabas.</p>
                    <div className="mt-10 flex items-center justify-center gap-x-6">
                        <a
                            href="/"
                            className="rounded-md bg-[#322017] px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#4a372d] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 duration-300"
                        >
                            Volver
                        </a>
                    </div>
                </div>
            </main>
        </>
    )
}
